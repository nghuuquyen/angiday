...
const BATCH_NAME = 'keyword_daily_import_to_neo4j'
...
async function start() {
  ...
  let totalRecords = await Keyword.count(queryConds);
  ...
  while (totalRecords > 0) {
    ...
    let records = await Keyword.find(queryConds)
...
function buildInserString(records) {
  ...
  for (let i in records) {
    let keyword = records[i];
    let batchItemStr = `{
      id: '${keyword.id}',
      name: '${_.escape(keyword.name)}'
    } `;
  ...
  batchInserStr += `
    ...
    MERGE (n:Keyword { id: batch.id })
    SET n.name = batch.name
  `;
